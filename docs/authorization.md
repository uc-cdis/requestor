...
- Providing a `username` in the request body creates the access request for that user: `create` access on a resource path allows filing requests for other users to the access for that resource. See [Requesting access on behalf of other users](on_behalf_authz.md) for an optional design idea on restricting this further. But note that even without that optional design: this is intended functionality as-is. Providing `create` for the `requestor` service on a resource is explicitly granting that user the ability to file requests for other users to access that resource. Note that the actual review and decision of such access is out of band of this service.
